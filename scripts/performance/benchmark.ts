...
import type { MovieStateMap } from "../../src/types";
...
for (const [movieCount, profileCount] of [[10_000, 1000], [50_000, 10_000], [100_000, 50_000]]) {
  ...
  const updatedStates: MovieStateMap = { ...fixture.states, "synthetic-0": { ...fixture.states["synthetic-0"], rating: 5, watched: true, watchlist: false } };
  runRecommendationJob({ ...fixture, states: updatedStates, minimumMovieYear: 1990 });
  ...
  const row = { movieCount, profileCount, recommendationMs: Math.round(recommendationMs), updateMs: Math.round(updateMs), searchMs: Math.round(searchMs), collectionMs: Math.round(collectionMs), recommendationCount: result.recommendations.length, sectionCount: result.sections.length, searchCount: search.movieIds.length };
  results.push(row);
...
if (process.argv.includes('--check') && results.some((row) => row.recommendationMs > 8000 || row.updateMs > 8000 || row.searchMs > 1000 || row.collectionMs > 1000)) {
