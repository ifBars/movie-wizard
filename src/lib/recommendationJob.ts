...
import { buildDiscoverSections, matchesRecommendationFocus } from "@/lib/discoverSections";
...
import type { Movie, MovieStateMap, Recommendation, RecommendationFocus, TasteProfile } from "@/types";
...
export function runRecommendationJob({ movies, states, minimumMovieYear, model, focus }: RecommendationJob): RecommendationJobResult {
  const snapshot = buildTasteSnapshot(movies, states, {
    minimumMovieYear,
    candidateFilter: (movie) => matchesRecommendationFocus(movie, focus),
  }, model);
